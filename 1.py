...
class MyPillow(QMainWindow):
    def __init__(self):
        ...
        self.filename = QFileDialog.getOpenFileName(self, 'Выберите картинку',
                                                    '', 'Картинки (*.jpg)')[0]
        ...
        for i in range(x):
            for j in range(y):
                ...
                if (self.sender().text() == 'R'):
                    ...
                    pass
        self.curr_image = self.curr_image.rotate(self.degree,
                                                 expand=True)
        # python 3.8 garbage collection issue
        self.a = ImageQt(self.curr_image)
        self.pixmap = QPixmap.fromImage(self.a)
    ...
    def rotate(self):
        if self.sender() is self.pushButton_6:
            ...
            degree = 90
        self.degree %= 360
        self.curr_image = self.curr_image.rotate(degree,
                                                 expand=True)
        # python 3.8 garbage collection issue
        self.a = ImageQt(self.curr_image)
        self.pixmap = QPixmap.fromImage(self.a)
